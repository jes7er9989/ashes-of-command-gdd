...
const AudioEngine = (() => {
  ...
  /* ═══════════════════════════════════════════════════════════
     NAMED ACTION SOUNDS — Tiered data-driven dispatch
     ═══════════════════════════════════════════════════════════ */

  /* ── Tier 1: Single _tone() calls ──────────────────────── */
  var SIMPLE_TONES = {
    /* Search */
    searchClose:    ['sine', 900, 240, 0.26, 0.15, null, null, true, 0.0],
    searchResult:   ['sine', 1100, 1300, 0.20, 0.07, null, null, false, 0.15],
    /* Scroll */
    scrollTick:     ['sine', 2400, 1800, 0.04, 0.03, 3000, 'lowpass', false, 0],
    /* Map / Territory */
    territoryHover: ['sine', 800, 1000, 0.08, 0.06, null, null, false, 0.0],
    /* Dashboard Cards */
    cardHover:      ['sine', 1800, 2000, 0.06, 0.04, 3000, 'lowpass', false, 0.0],
    phaseHover:     ['sine', 1200, 1400, 0.08, 0.05, null, null, false, 0.0],
    /* UI Feedback */
    loading:        ['sine', 600, 800, 0.08, 0.15, null, null, false, 0.0],
    /* Tooltip */
    tooltip:        ['sine', 3000, 2600, 0.06, 0.03, 4000, 'lowpass', false, 0.0],
    /* Unit Sprite */
    statHover:      ['sine', 1900, 2100, 0.07, 0.03, 3000, 'lowpass', false, 0]
  };

  /* ── Tier 3: Extracted complex functions ───────────────── */

  /* ── Faction Enter Stings ────────────────────────────────── */
  function playFactionEnterSting(faction) {
    var stings = {
      terran: function() {
    ...
    (stings[faction] || function() { _tone('sine', 880, 440, 0.36, 0.22, null, null, true); })();
  }

  /* ── Filter (faction-keyed) ──────────────────────────────── */
  function playFilterSound(faction) {
    var fp = {
    ...
    (fp[faction] || function() { _tone('sine', 900, 700, 0.24, 0.07); })();
  }

  /* ── Dev Unlock Sting ────────────────────────────────────── */
  function playDevUnlockSting() {
  ...
  /* ── Tier 2: Multi-call sequences and faction-keyed logic ── */
  var actions = {
    /* Search */
    searchOpen: function() { _tone('sine', 260, 1100, 0.30, 0.22, null, null, true, -0.2); setTimeout(function() { _tone('sine', 400, 1500, 0.18, 0.18, null, null, true, 0.2); }, 28); },
    /* Accordion */
    accordionOpen: function() { var hz = { terran: 700, horde: 200, shards: 1400, revenant: 280, accord: 880, vorax: 480, guardians: 370 }; var fq = hz[currentFaction] || 550; _tone('sine', fq * 0.65, fq, 0.24, 0.12, null, null, false, -0.1); },
    accordionClose: function() { var hz = { terran: 700, horde: 200, shards: 1400, revenant: 280, accord: 880, vorax: 480, guardians: 370 }; var fq = hz[currentFaction] || 550; _tone('sine', fq, fq * 0.52, 0.20, 0.10, null, null, false, 0.1); },
    /* Filter / Sort */
    filter: function() { playFilterSound(currentFaction); },
    sort: function() { var bases = { terran: 440, horde: 220, shards: 880, revenant: 330, accord: 523, vorax: 370, guardians: 293 }; var waves = { terran: 'sawtooth', horde: 'square', shards: 'sine', revenant: 'triangle', accord: 'sine', vorax: 'square', guardians: 'triangle' }; var b = bases[currentFaction] || 440; var w = waves[currentFaction] || 'sine'; _arp([[b, 0], [b * 1.26, 0.055], [b * 1.587, 0.11]], 0.26, 0.10, w, (currentFaction === 'shards' || currentFaction === 'guardians')); },
    /* Sidebar */
    sidebarOpen: function() { _whoosh(0.25, 0.18, 400, 2000, -0.4); setTimeout(function() { _ping(1800, 0.12, 0.08, -0.2); }, 80); },
    sidebarClose: function() { _whoosh(0.20, 0.15, 2000, 400, -0.4); },
    sidebarToggle: function() { _grind(0.42, 0.06, 250, 40, -0.2); setTimeout(function() { _tone('triangle', 200, 120, 0.26, 0.10, 350, 'lowpass', false, 0.2); }, 40); },
    /* Navigation */
    navChapter: function() { _whoosh(0.18, 0.22, 300, 1800, 0.0); setTimeout(function() { _ping(2200, 0.14, 0.06, 0.15); }, 100); },
    navBack: function() { _whoosh(0.16, 0.18, 1800, 300, 0.0); setTimeout(function() { _thud(120, 0.20, 0.12); }, 60); },
    navDashboard: function() { _whoosh(0.22, 0.25, 200, 2400, 0.0); setTimeout(function() { _ping(1400, 0.16, 0.10, -0.2); _ping(2100, 0.12, 0.08, 0.2); }, 120); },
    /* Scroll */
    scrollTop: function() { _tone('sine', 800, 1600, 0.16, 0.12, null, null, false, 0.0); setTimeout(function() { _ping(2400, 0.10, 0.06, 0.1); }, 50); },
    /* Map / Territory */
    mapHover: function() { _ping(1600, 0.10, 0.08, 0.0); },
    mapClick: function() { _thud(200, 0.18, 0.15); setTimeout(function() { _ping(1200, 0.14, 0.10, 0.0); }, 40); },
    territorySelect: function() { _thud(160, 0.22, 0.18); _noise(0.14, 0.06, 800, 4, false, 0.0); setTimeout(function() { _tone('sine', 600, 900, 0.16, 0.12, null, null, true, 0.0); }, 30); },
    /* Galaxy */
    galaxyHover: function() { _bell(1760, [[0, 1.0], [+5, 0.4]], 0.08, 0.20, true, 0.0); },
    galaxyClick: function() { _shimmer(880, 0.16, 0.35); },
    /* Dashboard Cards */
    cardClick: function() { _ping(1400, 0.18, 0.10, 0.0); _noise(0.10, 0.03, 1200, 6, false, 0.0); },
    /* Faction Enter Stings */
    factionEnter: function() { playFactionEnterSting(currentFaction); },
    /* Dev Mode */
    devUnlock: function() { playDevUnlockSting(); },
    editOn: function() { _tone('sawtooth', 440, 440, 0.36, 0.15, 600, 'lowpass', false, -0.15); setTimeout(function() { _tone('sawtooth', 660, 660, 0.36, 0.15, 600, 'lowpass', false, 0.15); }, 140); },
    editOff: function() { _tone('triangle', 880, 440, 0.36, 0.22, 700, 'lowpass', true, -0.1); setTimeout(function() { _tone('sine', 660, 330, 0.22, 0.18, null, null, true, 0.1); }, 40); },
    /* Export */
    'export': function() { _arp([[523, 0], [659, 0.08], [784, 0.16], [1047, 0.26], [1319, 0.38]], 0.28, 0.22, 'sine', true); setTimeout(function() { _arp([[698, 0], [880, 0.08], [1047, 0.16], [1319, 0.26]], 0.20, 0.22, 'sine', true); }, 22); setTimeout(function() { _bell(2093, [[0, 1.0], [7, 0.55], [12, 0.35]], 0.16, 0.55, true, 0.0); }, 420); setTimeout(function() { _noise(0.12, 0.18, 5000, 2, true, 0); }, 650); },
    /* UI Feedback */
    success: function() { _shimmer(1047, 0.20, 0.30); },
    error: function() { _tone('square', 180, 120, 0.22, 0.11, 300, 'lowpass', false, 0); setTimeout(function() { _tone('square', 140, 90, 0.18, 0.10, 250, 'lowpass', false, 0); }, 110); },
    warning: function() { _tone('triangle', 440, 440, 0.20, 0.08, null, null, false, 0.0); setTimeout(function() { _tone('triangle', 440, 440, 0.16, 0.06, null, null, false, 0.0); }, 120); },
    loadComplete: function() { _ping(2000, 0.14, 0.08, 0.0); setTimeout(function() { _ping(2800, 0.10, 0.06, 0.15); }, 60); },
    /* Ctrl+K */
    ctrlK: function() { _tone('sine', 440, 880, 0.24, 0.09, null, null, true, -0.15); setTimeout(function() { _tone('sine', 880, 1320, 0.16, 0.08, null, null, true, 0.15); }, 55); },
    /* Unit Sprite */
    unitSprite: function() { _tone('sine', 1600, 2200, 0.20, 0.06, null, null, false, 0.2); setTimeout(function() { _tone('sine', 2200, 1600, 0.12, 0.05, null, null, false, -0.1); }, 55); },
    /* Glitch effect */
    glitch: function() { _glitch(0.14, 0.08); },
    /* Chapter Index */
    indexOpen: function() { _whoosh(0.20, 0.20, 300, 2200, 0.0); setTimeout(function() { _shimmer(1400, 0.10, 0.20); }, 100); },
    indexClose: function() { _whoosh(0.16, 0.15, 2200, 300, 0.0); },
    /* Audio Panel */
    panelOpen: function() { _ping(1200, 0.14, 0.08, 0.3); setTimeout(function() { _ping(1800, 0.10, 0.06, 0.3); }, 50); },
    panelClose: function() { _ping(1800, 0.10, 0.06, 0.3); setTimeout(function() { _ping(1200, 0.08, 0.06, 0.3); }, 50); },
    /* Combat/Strategy clicks */
    clickCombat: function() { _grind(0.42, 0.06, 250, 50, 0); setTimeout(function() { _tone('square', 160, 80, 0.25, 0.11, 350, 'lowpass', false, 0); }, 35); },
    clickStrategy: function() { _tone('sine', 1200, 1400, 0.18, 0.07, null, null, false, 0.15); setTimeout(function() { _tone('sine', 1400, 1600, 0.12, 0.06, null, null, false, -0.1); }, 45); },
    clickGalactic: function() { _tone('sine', 280, 560, 0.22, 0.14, null, null, true, -0.2); setTimeout(function() { _tone('sine', 560, 840, 0.14, 0.12, null, null, true, 0.2); }, 70); }
  };

  /* ── Dispatcher ────────────────────────────────────────── */
  function playAction(type) {
    if (!ctx) init();
    if (SIMPLE_TONES[type]) { _tone.apply(null, SIMPLE_TONES[type]); return; }
    if (actions[type]) { actions[type](); return; }
